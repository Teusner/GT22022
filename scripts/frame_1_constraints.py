...
class frame_1_constraints(ThreeDScene):
    def construct(self):
        base_0 = Basis()
        ...
        R = Rotation.from_euler("zyx", [np.pi/4, np.pi/4, 0]).as_matrix()
        ...
        self.wait(0.5)
        self.play(
            Rotate(base_1, PI/4, axis=R@np.array([1, 0, 0]), about_point=[0, 0, 0]),
            run_time=1, rate_func=rate_functions.there_and_back
        )
        self.play(
